...
from featurebased.dataset import get_Dataloader
from featurebased.PolygonMLP import PolygonPairClassifier
from featurebased.trainer import MLPTrainer
...
def run_trial(config: dict, trial: optuna.trial.Trial = None):
    try:
        ...
        with mlflow.start_run(run_name=name, nested=True if trial else False):
            ...
            logger.info("🧠 Initialise encoder...")
            ...
            logger.info(f"💾 Estimated model size: {model_size_mb:.2f} MB (float32)")
            ...
            for epoch in range(1, config["num_epochs"] + 1):
                ...
                if current_val_metric < best_val:
                    ...
                    logger.info(f"🎉 New best model saved for this trial (loss: {best_val:.4f})")
                    for key, value in results.items():
                        mlflow.log_metric(f"{key}_best", value, step=epoch)
                else:
                    epochs_no_improve += 1
                    logger.info(
                        f"Since {epoch - epochs_no_improve}. No improvement since epoch[{epochs_no_improve}/{config['early_stopping']['patience']}]")
            ...
            logger.info(f"✅ Training for trial completed! Best: {best_val:.4f}")
        ...
        logger.info("🧹 Starts Cleanup after Trial...")
        mlflow.end_run()
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(BASE_DIR, "checkpoints/mlp_config.yaml")
