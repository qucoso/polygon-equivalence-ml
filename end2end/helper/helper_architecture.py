...
class LearnableFourierFeatures(nn.Module):
    ...
    def forward(self, x_coords):
        """
        Accepts a tensor with coordinates of the form (B, N, 2).
        """
...
class SinusoidalMultiScaleLocEncoder(nn.Module):
    def __init__(self, loc_encoding_dim=8, mode: str = 'simple',
        ...
        super().__init__()
        """
            min_freq: lowest frequency in 1/°
            max_freq: highest frequency in 1/°, e.g. ~5600 for ~5m resolution
        """
...
class PolygonGraphBuilder(nn.Module):
    ...
    def forward(self, data):
        ...
        pos_for_encoding = data.pos.unsqueeze(0)  # [1, N, 2]
        data.x = self.loc_encoder(pos_for_encoding)[0]  # [N, output_dim]
