...
from helper.helper_architecture import CyclicRelativePosEncoding
...
class PairPolygonDataset(Dataset):
    ...
    def _build_graph_data(self, coordinates: torch.Tensor) -> Data:
        ...
        with torch.no_grad():
            cyclic_pe = self.pos_encoder(coordinates.unsqueeze(0)).squeeze(0)

        return Data(pos=coordinates, edge_index=edge_index, cyclic_pe=cyclic_pe)
    ...
    def __getitem__(self, index: int):
        ...
        if self.model_mode == "graph":
            ...
            with torch.no_grad():
                pe1 = self.pos_encoder(poly1.unsqueeze(0)).squeeze(0)
                pe2 = self.pos_encoder(poly2.unsqueeze(0)).squeeze(0)

            return poly1, poly2, pe1, pe2, int(anchor_id), int(other_id), strategy
...
def collate_pair_sequence(batch):
    poly1, poly2, pe1, pe2, poly1_i, poly2_i, strategy = zip(*batch)
    ...
    pe1_padded = pad_sequence(pe1, batch_first=True, padding_value=0.0)
    pe2_padded = pad_sequence(pe2, batch_first=True, padding_value=0.0)
    ...
    return {
        ...
        "poly1_pe": pe1_padded, "poly2_pe": pe2_padded,
...
def get_dataloader(config, model_mode="graph"):
    ...
    model_key = "graph_encoder" if model_mode == "graph" else "perceiver_encoder"
    ...
    full_dataset = PairPolygonDataset(
        ...
        augmenter=augmenter,
        d_pos_enc=config[model_key].get("d_pos_enc", 16)
    )
...
class EvaluationPolygonDataset(Dataset):
    def __init__(self, parquet_path: str, model_mode: str = "graph", d_pos_enc: int = 16):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, str]:
        ...
        if self.model_mode == "graph":
            ...
            with torch.no_grad():
                cyclic_pe = self.pos_encoder(polygon.unsqueeze(0)).squeeze(0)
            
            polygon = Data(x=polygon, pos=polygon, edge_index=edge_index, cyclic_pe=cyclic_pe)
            return polygon, polygon_id
        else:
            with torch.no_grad():
                polygon = self.pos_encoder(polygon.unsqueeze(0)).squeeze(0)

            return polygon, cyclic_pe, polygon_id

def eval_collate_fn_seq(batch):
    polygons, pes, polygon_ids = zip(*batch)
    ...
    pes_padded = pad_sequence(pes, batch_first=True, padding_value=0.0)
