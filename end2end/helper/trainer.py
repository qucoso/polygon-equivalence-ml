...
class EmbeddingTrainer:
    ...
    def train_epoch(self, dataloader):
        """Performs a single training epoch with gradient accumulation and mixed precision."""
        self.encoder.train()
        total_loss = 0.0
        ...
        for batch_idx, batch in enumerate(loader):
            if self.model_mode == "graph":
                ...
                keys = ["poly1", "poly2", "poly1_pe", "poly2_pe", "poly1_mask", "poly2_mask", "poly1_i", "poly2_i"]
                poly1, poly2, poly1_pe, poly2_pe, poly1_mask, poly2_mask, poly1_i, poly2_i = [batch[_].to(self.device, non_blocking=True) for _ in keys]
                model_inputs = [(poly1, poly1_pe, poly1_mask), (poly2, poly2_pe, poly2_mask)]
    ...
    def evaluate(self, dataloader, k_values=None):
        """Evaluates encoder using cosine similarity; computes relevant metrics"""
        self.encoder.eval()
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                # Separate the distances into positive and negative pairs
    ...
    def _calculate_best_threshold_metrics(self, pos_dists: torch.Tensor, neg_dists: torch.Tensor):
        """Calculates metrics for best threshold based on distances"""
